...
class DataGenerator(keras.utils.Sequence):
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        for i, ID in enumerate(list_IDs_temp):
            ...
            x, useless  = np.squeeze(librosa.load(train_dir+ID, sr=sr))
            X[i,] = preprocess(x)
            #X[i,] = np.load(train_spectrum_mic_ex_dir+ID+'.npy')[0].reshape(fre_bin, max_time, 1)
...
def preprocess(sig):
    ...
    return X.reshape(fre_bin, max_time, 1)
